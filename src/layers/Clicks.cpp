  ...
  if (Mod::get()->getSettingValue<bool>("only-on-jump")) {
    ...
    if (Mod::get()->getSettingValue<bool>("enable-clicksound") && !usingCustomClickSound) {
      ...
      if (Mod::get()->getSettingValue<bool>("use-sfx-volume")) {
          ...
          log::debug("Click Sound Volume (sfx slider parse): {}", std::fmod(fae->m_sfxVolume, 100.f));
          log::debug("Click Sound Volume (sfx game manager slider): {}", GameManager::sharedState()->m_sfxVolume);
          log::debug("Click Sound Volume (sfx slider game manager parse): {}", std::fmod(GameManager::sharedState()->m_sfxVolume, 100.f));
        #else
