...
class buttonfunctionthing : public CCLayer {
  ...
  void onButtonClick(CCObject* sender) {
    FLAlertLayer::create(
      "About",
      "This badge is given to <cp>Click Sound developers/contributers</c>. If you have programmed anything that was added to Click Sound, You will obtain this badge. ^w^",
      "OK"
    )->show();
  }
};
...
class $modify(ProfilePage) {
  bool init(int p0, bool p1) {
    ...
    std::set<int> csContributors = {13873867, 14284530, 21207551, 25397826, 6253758, 1941705, 17389494};
    ...
    if (csContributors.count(p0) > 0) {
      auto btn = CCMenuItemSpriteExtra::create(
        spr,
        this,
        menu_selector(buttonfunctionthing::onButtonClick)
      );
